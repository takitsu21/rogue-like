...
public class Salle {
    ...
    private void createSalle() {
        for (int j = 0; j < lignes; j++) {
            representation[j][0] = new Case("#", null, CaseTypeEnum.WALL);
        }
        for (int i = 1; i < colonnes; i++) {
            representation[0][i] = new Case("#", null, CaseTypeEnum.WALL);
            for (int k = 1; k < lignes; k++) {
                representation[k][i] = new Case(" ", null, CaseTypeEnum.SALLE);
            }
            representation[lignes - 1][i] = new Case("#", null, CaseTypeEnum.WALL);
        }
        for (int j = 0; j < lignes; j++) {
            representation[j][colonnes - 1] = new Case("#", null, CaseTypeEnum.WALL);
        }
    }
