...
import com.mady.utils.entities.AbstractMonster;
import com.mady.utils.entities.Boss;
import com.mady.utils.entities.Entities;
import com.mady.utils.entities.Player;
import com.mady.utils.enums.CaseTypeEnum;
import com.mady.utils.environment.generation.AStar;
import com.mady.utils.environment.generation.PairPos;
...
import com.mady.utils.items.PaidChest;
import com.mady.utils.items.Trap;
...
public class Map {
        ...
        for (int i = 0; i < solvedPath.length; i++) {
            for (int j = 0; j < solvedPath[i].length; j++) {
                if (solvedPath[i][j] != -1 && !map[i][j].isSalle()) {
                    ...
                    map[i][j].setCt(CaseTypeEnum.PATH);
                }
            }
        }
        if (portes.size() == 2) {
    ...
    public void clearCase(Position pos) {
        map[pos.getX()][pos.getY()] = new Case(CaseTypeEnum.SALLE);
    }
...
//        System.out.printf("test move");
    ...
    private void generatePortalshop() {
        ...
        map[pos.getX()][pos.getY()] = new Case("$", CaseTypeEnum.SHOPPORTAL);
